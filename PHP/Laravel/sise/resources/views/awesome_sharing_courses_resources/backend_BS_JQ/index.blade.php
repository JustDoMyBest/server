...
<head>
	...
	<title>精品课程资源共享后台管理系统 0.1</title>
	<link href="{{ asset('sise/backend/style/authority/main_css.css') }}" rel="stylesheet" type="text/css" />
	<link href="{{ asset('sise/backend/style/authority/zTreeStyle.css') }}" rel="stylesheet" type="text/css">
	<script type="text/javascript" src="{{ asset('sise/backend/scripts/jquery/jquery-1.7.1.js') }}"></script>
	<script type="text/javascript" src="{{ asset('sise/backend/scripts/zTree/jquery.ztree.core-3.2.js') }}"></script>
	...
	// 			alert("父类节点无法点击哦...");
		...
		function loadMenu(resourceType, treeObj){
			...
			@include('awesome_sharing_courses_resources.backend_BS_JQ.layouts.nav')
            // 如果返回数据不为空，加载"业务模块"目录
            if(data != null){
                // 将返回的数据赋给zTree
                $.fn.zTree.init($("#"+treeObj), setting, data);
...
<body onload="getDate01()">
    <div id="top">
		<div id="top_logo">
            ...
            <font size='4' color='white'>精品课程资源共享后台管理系统 0.1</font>
		</div>
		<div id="top_links">
			<div id="top_op">
				<ul>
					...
					<li>
						<img alt="今天是" src="{{ asset('sise/backend/images/common/date.jpg') }}">：
			...
			<div id="top_close">
				<a href="javascript:void(0);" onclick="logout();" target="_parent">
					<img alt="退出系统" title="退出系统" src="{{ asset('sise/backend/images/common/close.jpg') }}" style="position: relative; top: 10px; left: 25px;">
				</a>
			</div>
		</div>
	</div>
	...
	<div id="side">
		 <div id="left_menu_cnt">
		 	<div id="nav_module">
		 		<img src="{{ asset('sise/backend/images/common/module_1.png') }}") width="210" height="58"/>
		 	</div>
		 	<div id="nav_resource">
	...
	<script type="text/javascript">
		$(function(){
			$('#TabPage2 li').click(function(){
				...
				$(this).find('img').attr('src', "{{ asset('sise/backend/images/common/') }}"+ (index+1) +'_hover.jpg');
				$(this).css({background:'#fff'});
				$('#nav_module').find('img').attr('src', "{{ asset('sise/backend/images/common/module_') }}"+ (index+1) +'.png');
				$('#TabPage2 li').each(function(i, ele){
					if( i!=index ){
						$(ele).find('img').attr('src', "{{ asset('sise/backend/images/common/') }}"+ (i+1) +'.jpg');
		...
		function switchSysBar(flag){
			...
			if( flag==true ){	// flag==true
		        ...
		        if ( left_menu_cnt.is(":visible") ) {
		        	...
		        	$('#main').css({left:'60px'});
		        	$("#show_hide_btn").find('img').attr('src', "{{ asset('sise/backend/images/common/nav_show.png') }}");
		        } else {
					left_menu_cnt.show(500, 'linear');
					side.css({width:'280px'});
					$('#top_nav').css({width:'77%', left:'304px', 'padding-left':'0px'});
		        	$('#main').css({left:'280px'});
		        	$("#show_hide_btn").find('img').attr('src', "{{ asset('sise/backend/images/common/nav_hide.png') }}");
		        }
			}
		}
	</script>
    ...
    <div id="main">
      	...
      	{{-- <iframe name="right" id="rightMain" src="{{ view('awesome_sharing_courses_resources.backend_BS_JQ.introduce') }}" frameborder="no" scrolling="auto" width="100%" height="100%" allowtransparency="true"/> --}}
      	<iframe name="right" id="rightMain" src="{{ url('introduce') }}" frameborder="no" scrolling="auto" width="100%" height="100%" allowtransparency="true"/>
      	{{-- @yield('iframe') --}}
    </div>
